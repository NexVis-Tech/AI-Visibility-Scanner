...
class Scoring_Engine {
	...
	/**
	 * Compute scores for a specific page in a given scan.
	 *
	 * @param int $scan_id
	 * @param int $post_id
	 * @return array
	 */
	public function calculate_page_score( int $scan_id, int $post_id ): array {
		global $wpdb;
		$table_results = $wpdb->prefix . 'avs_check_results';

		$results = $wpdb->get_results(
			$wpdb->prepare( "SELECT category, result FROM {$table_results} WHERE scan_id = %d AND post_id = %d", $scan_id, $post_id )
		);
